...
<Purpose>
  This script will attempt to measure disk read rate (generally meant for
  windows, but it would probably work anywhere?).  Current plan being 
  attempted:
    Time how long it takes to write 16KB, use that to determine write rate.
    Time how long it takes to read back that 16KB, use that to determine
    read rate. 
"""
...
def main():
  write_rate, junk_fn = measure_write(16*1024)
  os.remove(junk_fn)
