...
if callfunc == 'initialize':
  ...
  fullresourcedata = nmclient_rawsay(nmhandle, "GetVesselResources", myvessel)
  startingrestrictions = get_restrictions(fullresourcedata)
  
  # Set the restrictions to be nothing
  nmclient_signedsay(nmhandle, "SetRestrictions", myvessel, '')

  fullresourcedata = nmclient_rawsay(nmhandle, "GetVesselResources", myvessel)
  newrestrictions = get_restrictions(fullresourcedata)


  # Set the restrictions to be nothing
  nmclient_signedsay(nmhandle, "SetRestrictions", myvessel, startingrestrictions)

  if newrestrictions.strip() != '':
    print "'"+newrestrictions.strip()+"'"
    raise Exception, "Could not set restrictions!"

  
  fullresourcedata = nmclient_rawsay(nmhandle, "GetVesselResources", myvessel)
  finalrestrictions = get_restrictions(fullresourcedata)

  if finalrestrictions.strip() != startingrestrictions.strip():
    print "'"+finalrestrictions.strip()+"' != '"+startingrestrictions.strip()+"'"
    raise Exception, "Could not restore restrictions!"
