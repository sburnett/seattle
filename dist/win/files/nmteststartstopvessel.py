...
if callfunc == 'initialize':
  ...
  # stop it so that we can start it...
  if vesselinfo['vessels'][myvessel]['status'] == 'Started':
    nmclient_signedsay(nmhandle, "StopVessel", myvessel)
    vesselinfo = nmclient_getvesseldict(nmhandle)
    sleep(1)
    if vesselinfo['vessels'][myvessel]['status'] == 'Started':
      raise Exception, "Vessel is still started after request to stop it (is this due to lagging status updates)?..."
    
    
  nmclient_signedsay(nmhandle, "AddFileToVessel", myvessel, "infloop.py","""while True:
  sleep(.1)""")

  nmclient_signedsay(nmhandle, "StartVessel", myvessel, "infloop.py")
  ...
  # we'll stop and then look at the status because we want it to be stopped
  # when exiting regardless of whether or not it worked correctly...
  nmclient_signedsay(nmhandle, "StopVessel", myvessel)

  if vesselinfo['vessels'][myvessel]['status'] != 'Started':
    raise Exception, "Vessel is not started after request (is this due to lagging status updates)?..."
  ...
  if vesselinfo['vessels'][myvessel]['status'] != 'Stopped':
    raise Exception, "Vessel is not stopped after request (is this due to lagging status updates)?..."
