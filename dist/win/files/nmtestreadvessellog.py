...
if callfunc == 'initialize':
  ...
  newstring = str(randomfloat())

  # I need to double backslash because the program should contain \n
  programtext = "print '\\n"+newstring+"',\nwhile True:\n  sleep(.1)"


  nmclient_signedsay(nmhandle, "AddFileToVessel", myvessel, "infloop.py",programtext)

  nmclient_signedsay(nmhandle, "StartVessel", myvessel, "infloop.py")
  nmclient_signedsay(nmhandle, "StopVessel", myvessel)

  thelog = nmclient_signedsay(nmhandle, "ReadVesselLog", myvessel)
  if newstring != thelog.split('\n')[-1]:
    raise Exception, "The log '"+thelog+"' does not match the expected string '"+newstring+"'"
