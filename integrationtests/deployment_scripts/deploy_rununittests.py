...
def main():
  """
  <Purpose>
    Deploys the rununittests.py along with all the files needed into a
  ...
  <Usage>
    ...
    PYTHONPATH=$PYTHONPATH:/home/<user_name>/trunk && deploy_rununittests.py <target_folder>
  ...
  if cron_setup:
    ...
    cron_line="45 * * * * export GMAIL_USER='seattle.devel@gmail.com' && export GMAIL_PWD='repyrepy' && /usr/bin/python " + cron_tab_dir + "/rununittests.py >> " + cron_log_dir + "/cron_log.rununittests 2>&1" + os.linesep
...
def help_exit(exit_msg):
  ...
  print "python deploy_rununittests.py [option] <target_directory> [<log_directory>]"
