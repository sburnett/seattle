...
def main():
  """
  <Purpose>
    Deploys the ping_machines.py along with all the files needed into a
  ...
  <Usage>
    ...
    PYTHONPATH=$PYTHONPATH:/home/<user_name>/trunk && deploy_ping_machines.py <target_folder>
  ...
  preparetest.copy_to_target("../trunk/integrationtests/ping_machines/*", target_dir)
  ...
  if cron_setup:
    ...
    cron_line="25 * * * * export GMAIL_USER='seattle.devel@gmail.com' && export GMAIL_PWD='repyrepy' && /usr/bin/python " + cron_tab_dir + "/ping_machines.py >> " + cron_log_dir + "/cron_log.ping_machines 2>&1" + os.linesep

    #setup the cron job
    setup_crontab.add_crontab(cron_line, "ping_machines")
...
def help_exit(exit_msg):
  ...
  print "python deploy_ping_machines.py [option] <target_directory> [<log_directory>]"
