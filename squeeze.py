...
<Purpose>
  With this script, we can generate one file, portable, compressed
  versions of our popular scripts like Repy, Seash, and Repypp.

<Change log>
  I had to fix a few bugs in this unmaintained, 1998 library:

   * Dos and Unix line endings.  To use Python's compile() all lines must
     end with '\n'.  According the the Python docs, 
     {{{
       Note
       When compiling a string with multi-line statements, line endings 
   ...
   * Namespace collision between Repy's misc module and Python's misc module
     (from the compiler package).  I use a horrible hack for now.

   * Use of __name__ = '__main__'.  We want the main module to execute as if
     it's been called as a script.  I set the main module name to '__main__'.

<Original Readme>
  For more information on this library, see:
    http://effbot.org/zone/squeeze.htm

  This utility works for Python 1.4 and later.
...
class Loader(ihooks.ModuleLoader):
    ...
    def find_module(self, name, path = None):
        try:
            self.__modules[name]
	    if self.__last == 'compiler' and name == 'misc':
  	      raise KeyError 
  ...
  #    try:
            ...
            return m
#      except KeyError:
 #           return ihooks.ModuleLoader.load_module(self, name, (None, None, (None, None, BUILTIN_MODULE)))
