...
<Purpose>
  ...
  This file contains view functions for the download application which
...
def __get_guser__(www_username):
    """
   <Purpose>
      Returns a GENI user record by looking up the user via their
      username/www_username login into the GENI portal.
   ...
   <Returns>
      On success returns (geni_user, True) where geni_user is a User
      record from control/models. On failure, returns (HttpResponse,
      False) where HttpResponse represents the HTTP response to return
      to the user to explain what went wrong.

   <FixMe>
      We want templates for the error messages returned as
      HttpResponse objects, not raw HTML strings.
    """
    try:
